...
const messages: StatusMessage[] = [
...

    {
        ...
        template: async (bot: Bot) => `${(await bot.db.client.from(bot.db.users.collectionName("users")).select("*", { count: "estimated" })).count ?? "⛔"} users`
    },

    {
        ...
        template: async (bot: Bot) => {
            /* Total guild count */
            const guilds: number = ((await bot.client.cluster.fetchClientValues("guilds.cache.size")) as number[])
                .reduce((value, count) => value + count, 0);
